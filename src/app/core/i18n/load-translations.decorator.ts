...
export function LoadTranslations(scope: string, alias?: string): ClassDecorator {
	return (target) => {
		...
		/* Decorators run before the component definition exists when the class is
		   compiled ahead of time, so stash it and let Angular pick it up from the
		   static providers the component already declares. */
